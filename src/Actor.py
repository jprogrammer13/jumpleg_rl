...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        if action_dim > 1:
            ...
            action = torch.reshape(torch.cat((T_th, ComF_x, ComF_y, ComF_z, ComFd), axis=1),
                                   [action_shape, self.action_dim + 1])
        else:
            ...
            action = torch.cat((T_th[None, :], ComF_x[None, :], ComF_y[None, :], ComF_z[None, :], ComFd[None, :]),
                               axis=1)
